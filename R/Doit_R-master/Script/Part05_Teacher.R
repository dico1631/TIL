...
dim(exam)       # 행, 열 출력
(num_row <- dim(exam)[1]) # dim의 1은 row 수
str(exam)       # 데이터 속성 확인
exam$id
...
dim(mpg)      # 행, 열 출력
str(mpg)      # 데이터 속성 확인
summary(mpg)  # 요약 통계량 출력, 범주형 변수는 count
...
prop.table(table(mpg$test)) # 빈도 비율
...
midwest
head(midwest, 10)     # Raw 데이터 앞부분
tail(midwest)     # Raw 데이터 뒷부분
View(midwest)     # Raw 데이터 뷰어창에서 확인
dim(midwest)      # 차원
str(midwest)      # 속성
summary(midwest)  # 요약 통계량
...
#4번
# 각 도시의 인구수에 따른 가중치가 반영이 안되어서 이건 잘못된 것 > 책의 잘못
mean.asian.ratio <- mean(midwest$asian.ratio)
mean.asian.ratio

# 가중평균을 주어야한다.
mean.asian.ratio <- sum(midwest$asian)/sum(midwest$total) * 100
mean.asian.ratio

#그래프를 보면 이상치가 있음을 확인할 수 있다.
qplot(total, asian.ratio, data = midwest)


midwest$asian.grade <- ifelse(midwest$asian.ratio > mean.asian.ratio, "large", "small")
