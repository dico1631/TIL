...
# 데이터 불러오기
raw_welfare <- read.spss(file = "C:/Users/student/Desktop/TIL/R/Doit_R-master/Data/Koweps_hpc10_2015_beta1.sav",to.data.frame = T)
...
# factor : 숫자로 기록하지만 그 숫자와 인덱스가 같은 문자로 바꿔서 보여주게 만드는 것
...
# - 나머지는 "yes"로 설정한 후,
str(welfare$income)
economic_income <- welfare %>%
  select(age, income) %>%
  filter(age >= 20 & age <= 60) %>%
  mutate(isincome = ifelse(is.na(income), "no_working", "working")) %>%
  group_by(isincome) %>%
  summarise(working_num = n()) %>%
  mutate(NOworking_rate = working_num / sum(working_num) * 100)

economic_income


# - 20살에서 60살까지의 연령대에 대해 이에 대한 빈도수를 계산해보자.
age_isworking <- welfare %>%
  select(age, income) %>%
  filter(age >= 20 & age <= 60) %>%
  mutate(isincome = ifelse(is.na(income), "no_working", "working"),
         age10 = age %/% 10 * 10) %>%
  group_by(age10, isincome) %>%
  summarise(n = n()) %>%
  mutate(rate = n/sum(n))
...
ggplot(age_isworking, aes(isincome, rate, group=age10, fill = isincome)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = scales::percent(..y..), vjust = -.5)) +
  facet_grid(~age10) + 
...
sex_income = welfare %>%
  ...
  group_by(sex) %>%
  summarise(sex_mean = mean(income))
...
# Quiz : 20대 30대 40대 50대 60대 이상으로 분류해보시오. 10대는 제거하고 수입의 평균을 구하라.
aged <- welfare %>%
  filter(age >= 20) %>%
  mutate(aged = age %/% 10 * 10,
         aged = ifelse(aged >= 60, 60, aged)) %>%
  group_by(aged) %>%
...
SA_income <- welfare %>%
  ...
  filter(!is.na(income)) %>%
  mutate(aged = age %/% 10 * 10, aged = ifelse(aged >= 60, 60, aged)) %>%
  group_by(aged, sex) %>%
  summarise(mean_income = mean(income))

SA_income

ggplot(SA_income, aes(sex, mean_income, group = aged, fill = sex)) +
  geom_col() +
  geom_text(aes(label = round(mean_income), vjust = -.5)) +
  facet_grid(~aged)
...
ggplot(SA_income, aes(age, mean_income, col=sex)) + geom_line()
