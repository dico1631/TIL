...
condition <- (df_exam$sum >= 150) & (df_exam$sum <= 180)
df_exam[condition,]
...
df_exam[which.min(df_exam$science),]

minidx = tapply(df_exam$science, df_exam$class, which.min, simplify = F)
maxidx = tapply(df_exam$science, df_exam$class, which.max, simplify = F)
