...
# x, y, alpha(투명도), color, fill, shape, size
...
b + geom_bar(aes(Species), stat='count') # x만 지정된 경우 stat의 default 값이 count
...
iris %>%
  group_by(Species) %>%
  summarise(sumSL = sum(Sepal.Length))
...
ggplot(iris, aes(Species, Sepal.Length/sum(Sepal.Length), fill=Species)) + 
  geom_bar(stat = "identity") + 
  scale_y_continuous(labels=scales::percent) +  # y는 연속형인데 이것에 대한 라벨을 %로 줘
  ylab("relative frequencies")   # y축 이름 지정

## - bar 챠트 그리기(3): 범주 + 빈도에서 범주 순서 선택하기
ggplot(iris, aes(Species, Sepal.Length, fill=Species)) + 
  geom_bar(stat = "identity") +
  scale_x_discrete(limits = c("virginica", "versicolor", "setosa")) # x는 범주형이라 discrete

## - bar 챠트 그리기(4): 범주 + 그룹 + 빈도에서 누적 빈도 그리기
# Species-범주, variable-그룹, value-빈도
...
library(reshape2)
meanIris <- melt(meanIris, id.vars=c("Species")) # melt : wide > long data
meanIris

# position default : stack > 위에 쌓기
ggplot(meanIris, aes(Species, value, fill=variable)) + 
  geom_bar(stat = "identity")

# position : dodge > 뒤에 꼬리물고 연결
ggplot(meanIris, aes(Species, value, fill=variable)) + 
  geom_bar(stat = "identity", position = 'dodge')

## - bar 챠트 그리기(4): 범주 + 그룹 + 빈도에서 facet grid 그리기
# ..A..은 ggplot의 문법
# x는 day, y는 x로 계산한 값 / ..prop.. : percent / 
...
meanIris
ggplot(meanIris, aes(Species, value/sum(value), group=variable, fill = Species)) +
  geom_bar(stat = "identity") +
